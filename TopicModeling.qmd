...
### Movie Plots with Genre K-Means Clustering

Next, I will perform a k-means clustering of groups. This is a way to transform the document-topic gamma values into a format of clustering.

```{r}
#Honestly chat helped me with this... I do understand it though
gamma_wider <- gammas %>% pivot_wider(names_from = topic, values_from = gamma)
gamma_wider <- gamma_wider %>% drop_na()

cluster <- kmeans(gamma_wider %>% select(-document), centers = 10)

fviz_cluster(cluster, data = gamma_wider %>% select(-document), 
             geom = "point", stand = FALSE, 
             main = "K-means Clustering of Movies")
```
